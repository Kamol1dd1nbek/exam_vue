    ...
    <div>
        <div class="row position-sticky top-0 z-3 bg-light border d-flex">
            ...
            <div class="col-3 offset-4 d-flex align-items-center">
                <button @click="add" class="btn btn-outline-success form-control">Add</button>
            </div>
        </div>
        <div class="container">
            <ProductBox v-for="(item, index) in data" :key="index" :item="item"/>
        </div>
    </div>
</template>
...
watch(() => productStore.allProducts.data, () => {
    loading.value = productStore.allProducts.loading;
...
const add = () => {
    addProductModal.value.openModal();
}
</script>
